...
// Style code = everything BEFORE the first dash. The part after the dash is the
// age group and is ignored (all age groups share the same images).
//   "BLPTSW477-23" -> "BLPTSW477"     "BLPTSW477" -> "BLPTSW477"
export function styleCodeOf(raw) {
  const s = String(raw ?? '').trim();
  const i = s.indexOf('-');
  return (i > 0 ? s.slice(0, i) : s).trim();
}

// Only the style code is ever searched for as a folder name.
export function candidatesFor(raw) {
  return [styleCodeOf(raw)];
}
  ...
  for (const c of Object.keys(cand)) {
    ...
    if (!pick[c]) {
      results[c] = {
        ...
        message: `Folder "${cand[c][0]}" does not exist in Drive.`
      };
    } else {
