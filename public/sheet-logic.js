...
(function (root) {
  ...
  /**
   * Original columns + Image 1..N + Status.
   * N = the largest image count of any row (so 7, 8, 12... all work).
   */
  function buildOutput(header, rows, results, linkMode) {
    const perRow = rows.map((row) => {
      const code = codeOf(row);
      const res = results[code] || (code ? { status: 'Error', images: [], message: 'No result returned.' } : { status: 'Error', images: [], message: 'Empty style code.' });
      const links = (res.images || []).map((f) => linkFor(f.id, linkMode));
      return { row, code, res, links };
    });
    ...
    const statusHeader = uniq('Status');

    const aoa = [[...header, ...imgHeaders, statusHeader]];
    perRow.forEach(({ row, res, links }) => {
      const cells = Array.from({ length: maxImages }, (_, i) => links[i] || '');
      aoa.push([...row, ...cells, res.status]);
    });
    return { aoa, maxImages, origCols: header.length, perRow };
  }

  const api = { parseSheet, buildOutput, linkFor, codeOf };
